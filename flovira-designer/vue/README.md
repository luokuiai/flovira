...
<h1 align="center" style="margin: 30px 0 30px; font-weight: bold;">Flovira工作流 v1.2.7</h1>
<p align="center">
...
Flovira 国产工作流引擎，特点是简洁轻量、功能完整、扩展性强；后端通过 jar 集成引擎与设计器 API，前端通过本 npm 包集成设计器。
...
# 构建测试环境 bun run build:stage
# 构建生产环境 bun run build:prod
...
```bash
bun run build:lib   # 输出到 dist-lib/（ESM + 类型声明 .d.ts + 合并样式）
```
...
// UI 适配器从子入口按需引入；antdv 用 `@luokuiai/flovira-vue-designer/antdv` 的 `antdvAdapter`
...
- **注册时机**：`setUiAdapter` 必须在渲染 `FlowDesigner` 之前调用；`app.use(FloviraDesigner)` 会默认注册 element-plus 适配器。
...
宿主需提供：`vue`、`vue-router`、`pinia`、`@logicflow/core`、`@logicflow/extension`（必选）；UI 库三选一——`element-plus`（子入口 `@luokuiai/flovira-vue-designer/element-plus`）/ `ant-design-vue@4`（子入口 `@luokuiai/flovira-vue-designer/antdv`）/ `naive-ui`（子入口 `@luokuiai/flovira-vue-designer/naive`），均为可选 peer，按所选适配器 `setUiAdapter(...)`。图标已内置（iconify 离线集 `ep` + `wf`，随库打包，无需额外依赖）。

### 本地体验 playground

```bash
bun run build:lib        # 先产出 dist-lib
bun run dev:playground   # 启动最小消费示例（端口 5180，以包名 import dist-lib 产物）
```

包名固定为 `@luokuiai/flovira-vue-designer`；本地工作区通过 `workspace:*` 消费 `dist-lib` 产物。
