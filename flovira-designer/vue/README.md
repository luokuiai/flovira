...
业务表单使用字符串 `formId` 引用，表单结构和页面由接入方维护。基础信息设置流程表单，节点可覆盖，留空则继承。数据提供者的资源查询通过 `resourceType: 'FORM'` 返回 `{ id, name }` 选项；没有选项时可填写业务表单标识。旧 `formCustom` / `formPath` 和内置表单管理已移除，已有数据请按[迁移说明](../../docs/changes/2026-09-08-external-form-references.md)转换。
