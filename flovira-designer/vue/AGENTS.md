# AGENTS.md - Vue designer 模块规则

> 本文件只写 `flovira-designer/vue` 的差异化规则。先遵循上级 [`../AGENTS.md`](../AGENTS.md)，通用协作原则以仓库根 [`../../AGENTS.md`](../../AGENTS.md) 为准。
>
> 注意：本模块是根 Bun workspace 管理的**前端工程**，不受根 `AGENTS.md` 中 Java / JDK / ORM / SQL 规则约束；后端编译命令与本模块无关。

## 模块职责

`flovira-designer/vue` 是 Flovira 流程设计器的 **Vue 3 组件库 / npm 包工程**（包名 `@luokuiai/flovira-vue-designer`，经典 + 仿钉钉双模式）。本模块是纯组件库，不提供独立应用壳，也不生成供后端 jar 内嵌的网页。消费示例位于 `../examples/vue-element-plus`（:5180）、`../examples/vue-antdv`（:5181）和 `../examples/vue-naive`（:5182）。
...
- 包管理 **bun（workspace）**；`type: module`；**前端许可证为 MIT**（与后端 Apache 2.0 不同，勿混改 header）。
...
bun dev         # 库开发：vite build --config vite.lib.config.js --watch，持续重建 dist-lib
bun mock        # 零依赖 node mock HTTP 服务（:8080）

# 消费示例（在 flovira-designer/examples/<demo>/，经 workspace:* 消费库 dist-lib）
bun dev         # Element Plus :5180 / Ant Design Vue :5181 / Naive UI :5182
bun build       # 示例生产构建
```

**推荐开发循环（不依赖 npm 推送，可快速调试）**：一个终端在库目录 `bun dev`（watch 重建 dist-lib），另一终端在某 demo 目录 `bun dev`；改库源即重建产物并反映到 demo。
...
设计器对具体 UI 库（Element Plus / Ant Design Vue 等）的依赖已收敛到 `src/ui` 适配层，**主入口做到 UI 库无关、可按需切换**。
...
- **主入口 UI 库无关**：`designer/index.ts` 的 `install()` 只注册 svg-icon + 中性组件 `wf-*`，**不静态引、也不默认注册任何 UI 适配器**；主 bundle `dist-lib/flovira-designer.es.js` **零 element-plus / ant-design-vue import**（已实测 0 处）。消费方**必须**在渲染 `FlowDesigner` 前显式 `setUiAdapter(...)`，否则中性组件 `getUiAdapter()` 抛错。
- **EP / antd / naive 均为可选子入口（对称）**：
  - Element Plus：`src/ui/elementPlusAdapter.ts` → `vite.ep.config.js`（`build:ep`）→ `dist-lib/element-plus.es.js` → 包导出 `@luokuiai/flovira-vue-designer/element-plus`。
  - Ant Design Vue 4：`src/ui/antdvAdapter.ts` → `vite.antdv.config.js`（`build:antdv`）→ `dist-lib/antdv.es.js` → 包导出 `@luokuiai/flovira-vue-designer/antdv`。
  - Naive UI：`src/ui/naiveAdapter.ts` → `vite.naive.config.js`（`build:naive`）→ `dist-lib/naive.es.js` → 包导出 `@luokuiai/flovira-vue-designer/naive`。Naive 的 message/dialog/notification 是 provider 上下文式 API，适配器内用 `createDiscreteApi`（懒初始化）取脱上下文实例供命令式方法调用。
  - 三者各自 externalize `vue` + 对应 UI 库为 peer；`build:lib` = 主 + ep + antdv + naive **四构建**。消费方：`import { elementPlusAdapter } from '@luokuiai/flovira-vue-designer/element-plus'`（或 `/antdv` 的 `antdvAdapter`、`/naive` 的 `naiveAdapter`）+ `setUiAdapter(...)`。
...
- **构建**：纯库只有 `build:lib`（`dist-lib/`）；改 `vite.lib.config.js` 的 external / 入口后，跑 `build:lib` + 至少一个 demo `bun build` 验证消费正常。
- **LogicFlow 版本**：图引擎升级可能破坏既有流程图，升级按高风险处理并实际验证。
- **依赖版本**：锁定版本不随意升级；新增依赖先评估必要性与体积（尤其会进 `dist-lib` 的运行时依赖）。

## 验证

- 改动后至少库 `bun build:lib` 成功 + 至少一个 demo `bun build` 成功（验证包消费）；涉及流程图 / 表单 / UI 适配交互时，在 demo（ep :5180 / antd4 :5181）`bun dev` 手动 / playwright 验证关键路径。
