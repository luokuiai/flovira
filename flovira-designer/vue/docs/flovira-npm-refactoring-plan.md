# Flovira UI NPM 包改造完整实施方案

> **历史文档**：本文保留迁移背景和决策过程，其中 SPA/WebJar 双模方案已废止。当前结构见 [`../../README.md`](../../README.md)：后端不再提供内嵌页面，Vue 与 React 设计器作为独立 npm 包交付。
...
flovira-ui 当前通过 **SPA + iframe 嵌入** 方式集成：

**前端使用方式 (hh-vue/warm_chart.vue):**
```vue
<iframe :src="`${VUE_APP_FLOW_API}/flovira-ui/index.html?id=${insId}&type=FlowChart&Authorization=Bearer ${getToken()}`" />
```

**后端静态资源映射 (FloviraUiConfig.java):**
```java
@Override
public void addResourceHandlers(ResourceHandlerRegistry registry) {
    registry.addResourceHandler("/flovira-ui/**")
        .addResourceLocations("classpath:/META-INF/resources/flovira-ui/", "classpath:/flovira-ui/");
}
```

**构建流程:**
```
flovira-ui (Vue3项目) → npm run build:prod → dist/ 
    → 手动复制到 flovira-plugin-vue3-ui/src/main/resources/flovira-ui/
    → Maven 打包进 JAR 的 META-INF/resources/
    → Spring Boot 启动后 ResourceHandler 映射 /flovira-ui/**
```
...
| 目标维度 | 描述 | 本文档覆盖 |
|---------|------|----------|
| **NPM Vue 3 组件模式** | `npm install @flovira/vue-designer` → 直接 `<FloviraDesigner>` 使用 | ✅ 核心 |
| **SPA 向后兼容** | 已有 iframe 方式保留不动，零破坏性变更 | ✅ 核心 |
| **配置注入体系** | API baseURL、Token 函数、请求拦截器等全部外部注入 | ✅ 核心 |
| **渐进淘汰 SPA** | 长期可移除 SPA 模式（删除成本 < 15 分钟） | ✅ 覆盖 |
| **Web Component 模式** | Vue2/React/Angular/原生JS 均可通过 `<flovira-designer>` 使用 | ⏭ Phase 2 (后续) |
...
| 框架 | 引入方式 | 示例 |
|------|---------|------|
| **Vue 3** | `import { FloviraDesigner } from '@flovira/vue-designer'` | `<FloviraDesigner :id="defId" @save="onSave" />` |
| **Vue 2** | `import '@flovira/designer'` | `<flovira-designer id="defId" />` |
| **React** | `import '@flovira/designer'` | `<flovira-designer ref={ref} />` + ref.addEventListener |
| **Angular** | `import '@flovira/designer'` | `<flovira-designer [id]="defId" (save)="onSave($event)" />` |
| **原生 JS** | `import '@flovira/designer'` | `document.createElement('flovira-designer')` |
...
export { FloviraDesigner } from './components/FloviraDesigner.vue'
export { FloviraChart } from './components/FloviraChart.vue'
...
export function getToken(key) {
  ...
  if (_customGetToken) {
    try {
      ...
      console.warn('[flovira] customGetToken error:', e)
    }
  }
...
-   return request({ url: urlPrefix + 'flovira-ui/config', method: 'get' })
+   return request({ url: getUrlPrefix() + 'flovira-ui/config', method: 'get' })
  }
```
...
| 行号(旧) | 原 API 路径 | 改造后 |
|----------|------------|--------|
| 8 | `urlPrefix + 'flovira/save-json'` | `getUrlPrefix() + 'flovira/save-json'` |
| 25 | `urlPrefix + 'flovira/query-def' + id` | `getUrlPrefix() + 'flovira/query-def' + id` |
| 33 | `urlPrefix + 'flovira/query-flow-chart/' + id` | `getUrlPrefix() + ...` |
| 41 | `urlPrefix + 'flovira/handler-type'` | `getUrlPrefix() + ...` |
| 49 | `urlPrefix + 'flovira/handler-result'` | `getUrlPrefix() + ...` |
| 57 | `urlPrefix + 'flovira/handler-feedback'` | `getUrlPrefix() + ...` |
| 65 | `urlPrefix + 'flovira/handler-dict'` | `getUrlPrefix() + ...` |
| 73 | `urlPrefix + 'flovira/published-form'` | `getUrlPrefix() + ...` |
| 81 | `urlPrefix + 'flovira/node-ext'` | `getUrlPrefix() + ...` |
| 89 | `urlPrefix + 'flovira/listener-list'` | `getUrlPrefix() + ...` |

**form.js 同理, 6 处替换:**

| 行号(旧) | 原 API 路径 |
|----------|------------|
| 8 | `urlPrefix + 'flovira/form-content/' + id` |
| 16 | `urlPrefix + 'flovira/form-content'` |
| 25 | `urlPrefix + 'flovira/execute/load/' + id` |
| 33 | `urlPrefix + 'flovira/execute/handle?...` |
| 40 | `urlPrefix + 'flovira/execute/hisLoad/' + taskId` |
...
理由：
1. 表单设计是 **flovira 的可选功能**（非所有用户都用到）
...
| N4 | `src/core/install.js` | ~30 | Vue plugin 安装函数 (app.use(FloviraPlugin)) |
...
| N9 | `src/components/FloviraDesigner.vue` | ~80 | 流程设计器包装组件 |
| N10 | `src/components/FloviraChart.vue` | ~50 | 流程图查看器包装组件 |
...
> **用途**: 提供 Vue Plugin 形式的安装方式, 用户可 `app.use(FloviraPlugin, config)` 全局注册。

```javascript
// ====== 新增文件: src/core/install.js ======
import { initFlovira } from './config.js'
 ...
 *
 * 使用方式:
 *   import FloviraPlugin from '@flovira/vue-designer'
 *   app.use(FloviraPlugin, {
 *     baseURL: '/api',
...
function install(app, options = {}) {
  // 1. 合并配置
  const config = initFlovira(options)
  ...
  // 5. 注册全局组件 (可选便捷使用)
  //    用户也可以按需 import 单个组件
  app.component('FloviraDesigner', () =>
    import('../components/FloviraDesigner.vue')
  )
  app.component('FloviraChart', () =>
    import('../components/FloviraChart.vue')
  )
  ...
  // 6. 提供全局配置访问
  app.provide('$floviraConfig', config)
}
...
// (SPA 模式下可通过 window.__floviraAppStore 访问)
if (typeof window !== 'undefined') {
  window.__floviraAppStore = appStore
}
</script>
```
...
| 5 | 增加 `window.__floviraAppStore = appStore` | 调试便利, 非必需 |
...
export function setConfig(userConfig) {
  ...
  if (currentConfig.baseURL !== undefined || currentConfig.urlPrefix !== undefined) {
    ...
    try {
      ...
      console.warn('[flovira] config 级联初始化失败(非致命):', e.message)
    }
  }
...
export function initFlovira(config = {}) {
...
**步骤 4.1: FloviraDesigner.vue (~80 行)**
```vue
<!-- src/components/FloviraDesigner.vue -->
<template>
  <flow-design-index
    v-bind="$attrs"
    :on-close="onClose"
...
defineOptions({ name: 'FloviraDesigner' })
...
**步骤 4.2-4.4:** 同理创建 FloviraChart.vue / WarmFormDesigner.vue / WarmFormCreate.vue (各 40-50 行)
  ...
  return {
    ...
    build: isLib ? {
      ...
      lib: {
        ...
        name: 'FloviraDesigner',
        formats: ['es', 'umd'],
        fileName: (format) => `flovira-designer.${format}.js`,
      },
      rollupOptions: {
...
```diff
  {
-   "name": "Flovira",
+   "name": "@flovira/vue-designer",    // [CHANGED] scoped package name
    "version": "1.9.0",                     // [CHANGED] 大版本号升级
    "description": "Flovira 流程设计器 Vue 组件库",
    "type": "module",
    "scripts": {
-     "dev": "vite",
...
+   "main": "./dist-lib/flovira-designer.umd.js",        // [NEW] UMD 入口
+   "module": "./dist-lib/flovira-designer.es.js",        // [NEW] ES Module 入口
+   "exports": {                                              // [NEW] 子路径导出
+     ".": {
+       "import": "./dist-lib/flovira-designer.es.js",
+       "require": "./dist-lib/flovira-designer.umd.js",
+     },
...
```
dist-lib/
├── flovira-designer.es.js      # ES Module 格式 (~200KB gzipped?)
├── flovira-designer.umd.js      # UMD 格式
...
<script setup>
import { FloviraDesigner } from '@flovira/vue-designer'
import '@flovira/vue-designer/style.css'  // 引入样式!
...
| V5 | NPM import 组件 | 测试项目中 `<FloviraDesigner>` | 组件正常渲染 |
...
Phase A: 现在 ────────────────────── 双轨并行发布
│
│  ✓ npm publish @flovira/vue-designer v1.9.0
│  ✓ SPA dist/ 继续随 flovira-plugin 发布
...
│
│  ✗ 删除/归档 flovira-plugin-vue3-ui 模块
│  ✗ 删除 FloviraUiConfig.java
│  ✗ package.json 中移除 build:prod 脚本
│  ✗ 非 Vue 3 用户引导: CDN iframe 或自建网关
│
│  最终架构:
│  @flovira/designer (纯 NPM/WC 分发)
│  ├── vue3/     (Vue 3 组件)
│  ├── wc/       (Web Component)
...
| D7 | `flovira-plugin-vue3-ui/` | 目录 | 1min | 后端插件不再内嵌UI |
| D8 | `FloviraUiConfig.java` | 文件 | 1min | 静态资源映射移除 |
...
又使用 flovira NPM 组件:
  import '@flovira/vue-designer/style'   // flovira: 也包含 EP 样式子集
                                              → 样式重复! 选择器覆盖/优先级冲突风险
```
...
|----------|-------------------|------|
| flovira 业务样式 | ~10-20KB | `.wf-*` / `--wf-*` 变量 / 设计器布局 |
...
app.use(FloviraPlugin, {
  baseURL: '/api',
  elementPlusInstalled: true,  // [P21] 告诉 flovira "宿主已有EP样式, 不要再注入"
  getToken: () => localStorage.getItem('token')
})
// 此时 import '@flovira/vue-designer/style' 只导入业务样式, 不包含 EP 子集
...
> **注意**: 如果用户同时使用 FloviraDesigner + FloviraChart 两个组件, 体积会叠加共享部分 (LogicFlow/Element Plus 只算一次)。
