...
> **历史文档**：记录早期从内嵌 SPA 向 npm 包迁移的过程，不代表当前交付方式。当前结构见 [`../../README.md`](../../README.md)：后端不再提供内嵌页面，Vue 与 React 设计器作为独立 npm 包交付。

> **版本**: v1.0 | **基于**: flovira-npm-refactoring-plan.md v1.2 (21项问题已修复)
...
│         │                   │                      │             │
│         ▼                   ▼                      ▼             │
│  src/core/ 创建         npm publish           @flovira/        │
│  config/request/auth    @flovira/            vue-designer-vue2 │
...
| 因素 | 分析 | 结论 |
|------|------|------|
| **现有代码基础** | flovira-ui 已是纯 Vue 3 项目 | 先出 Vue 3 NPM 包成本最低 (~8h) |
    ...
    axiosInstance.interceptors.response.use(
      ...
      (error) => {
        if (error.response?.status === 401) {
          console.warn('[flovira] 401 未授权, 请检查 Token 配置')
        }
...
#### `src/components/FloviraDesigner.vue` (~60行)
...
| 组件名 | 内部引用视图 | 特殊 props |
|--------|------------|-----------|
| `FloviraChart.vue` | views/flow-design/flowChart.vue | id, instanceId, taskId, readOnly |
...
```json
{
  ...
  "main": "./dist-lib/flovira-designer.umd.js",
  "module": "./dist-lib/flovira-designer.es.js",
  "exports": {
    ".": {
...
// 用户: import { FloviraDesigner } from '@flovira/vue-designer'

// ===== 组件导出 =====
export { default as FloviraDesigner } from './components/FloviraDesigner.vue'
export { default as FloviraChart } from './components/FloviraChart.vue'
...
// ===== 配置 API =====
export { setConfig, getConfig, initFlovira, isLibraryMode } from './core/config.js'
...
```javascript
// src/core/install.js
// 功能: app.use(FloviraPlugin, options) 全局安装
...
npm run build:lib        # → dist-lib/ 应包含:
                          #   flovira-designer.es.js   (~150-250KB)
                          #   flovira-designer.umd.js   (~180-300KB)
                          #   style.css                   (~45-80KB)
...
import '@flovira/vue-designer/style.css'
...
```
Phase 2.1: 将 FloviraDesigner.vue 包装为 Custom Element (~1天)
  ↓
Phase 2.2: <flovira-designer> 标签可在任何框架中使用 (~1天)
  ↓
Phase 2.3: 发布 @flovira/designer (框架无关包) (~0.5天)
  ↓
...
import FloviraDesigner from './components/FloviraDesigner.vue'

const FloviraCE = defineCustomElement(FloviraDesigner)
customElements.define('flovira-designer', FloviraCE)

// 使用方式 (任意框架):
// <flovira-designer id="xxx" type="design"></flovira-designer>
```
...
```
@flovira/vue-designer      → Vue 3 版本 (Phase 1 产物)
@flovira/vue2-designer      → Vue 2 版本 (新仓库/分支, 用 vue2 + element-ui 重写视图层)
```
...
| `src/components/FloviraDesigner.vue` | ~60 | 设计器包装组件 |
| `src/components/FloviraChart.vue` | ~45 | 流程图包装组件 |
...
| **总计** | **Phase 0 + 1** | **~9h** | **@flovira/vue-designer v1.9.0** |
