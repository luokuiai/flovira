  ...
  <div class="user-select-wrapper">
    ...
    <div class="modern-tabs-wrapper" v-if="tabsList.length > 0">
      <div class="modern-tabs">
        ...
        >
          <span class="tab-label">{{ approverStrategyLabel(item) }}</span>
        </div>
      </div>
    </div>
        ...
        <div class="mobile-card-list mobile-only" v-loading="loading" @scroll="onMobileScroll">
          ...
          <div class="mobile-load-more">
            <template v-if="tableList.length > 0">
              ...
              <span v-else-if="mobileNoMore || tableList.length >= total" class="load-text load-end">{{ t('selectUser.loadedAll') }}</span>
            </template>
          </div>
          <!-- 空状态 -->
