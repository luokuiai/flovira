<template>
  <aside v-if="effectiveMode === 'panel' && drawer" class="property-panel-modern">
    <header class="property-panel-header">
      <div class="property-panel-title">
        <span class="property-panel-icon"><svg-icon icon-class="ep:setting" /></span>
        <span>{{ title }}</span>
      </div>
...
const effectiveMode = computed(() => props.displayMode === 'panel' && windowWidth.value > 1024 ? 'panel' : 'drawer');
...
interface PropertySettingProps {
  /** 属性编辑容器：panel 在桌面内联显示且窄屏自动回退 drawer。 */
...
const title = computed(() => {
  ...
  } else if (props.node && props.node.type === 'subProcess') {
