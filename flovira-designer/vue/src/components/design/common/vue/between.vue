  ...
  <div class="between">
    <wf-form ref="formRef" class="betweenForm" :model="form" label-width="110px" :rules="rules" :disabled="disabled">
      ...
      <div v-show="tabsValue === '1'" class="tabPane">
        ...
        <node-timeout ref="timeoutRef" v-model="form" node-type="between" :disabled="disabled" />
...
/** 查询节点扩展属性 */
async function getNodeExt() {
  const items = await designerResourceItems({ resourceType: 'NODE_EXTENSION', pageNum: 1, pageSize: 1000 });
  const extensions: any[] = items.map(item => ({ ...item.metadata, code: item.code, name: item.name }));
  extensions.forEach(e => {
        ...
        if (e.type === 1) baseList.value.push(...e?.childs);
          ...
          tabsList.value.push({ label: e.name, name: e.code, iconPath: TAB_ICONS.ext })
...
function validate() {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      await timeoutRef.value?.validate()
    } catch (_) {
      reject(false)
      return
    }
