...
import {designerSubjects} from "@/api/flow/definition";
...
interface BaseNodeProps {
  ...
  /** 描边色 */
  stroke?: string;
  subprocessSummary?: Record<string, any> | null;
...
const waitKey = computed(() => {
  try {
    return props.ext?.waitConfig ? JSON.parse(props.ext.waitConfig).waitKey || '' : ''
  } catch (_) {
    return ''
  }
})
...
watch(
    ...
    (newVal) => {
      if (newVal) {
        designerSubjects(newVal.split("@@")).then(subjects => {
