...
function addEdge(lf, nodeType, sourceNodeId, targetNodeId, pointsList = undefined) {
  return lf.addEdge({
    type: nodeType,
...
function addEdgeAll(lf, nodeType, sourceNodeId, targetNodeId, properties, text = undefined, pointsList = undefined) {
  return lf.addEdge({
    type: nodeType,
    sourceNodeId: sourceNodeId,
    targetNodeId: targetNodeId,
...
export const addBetweenNode = (lf, edge, nodeType = 'between', nodeName = '中间节点') => {
  const nodes = lf.getGraphData().nodes;
  const sourceNode = nodes.find(node => node.id === edge.sourceNodeId);
  const targetNode = nodes.find(node => node.id === edge.targetNodeId);

  // 添加中间节点
  const betweenNode = addNode(lf, nodeType, sourceNode.x, getMoveY(sourceNode, 'between'), nodeName)
