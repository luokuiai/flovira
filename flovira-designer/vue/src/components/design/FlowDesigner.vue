    ...
    <wf-header class="wf-designer-body" :style="headerStyle">
      ...
      <div class="wf-design-workspace" v-show="activeStep === 1">
        ...
        <div class="wf-canvas-stage">
          ...
          <div class="logo-text"><slot name="logo">Flovira</slot></div>
        </div>
...
const headerStyle = computed(() => {
  return {
    ...
    height: "auto",
    minHeight: "0",
    backgroundColor: "var(--wf-bg-container, #f5f7fa)",
...
const headerDiv = computed<CSSProperties>(() => {
    return {
      backgroundColor: "var(--wf-bg-container, #f5f7fa)",
      minHeight: "100%",
      height: "100%",
...
function handlePropertyVisibilityChange() {
  if (!lf.value) return;
  nextTick(() => {
    if (lf.value?.resize) lf.value.resize();
  });
}
...
/**
 * 收集当前可保存的 flovira 流程 json 字符串（命令式 API）。
 * 等价于保存前的数据装配：合并基础信息 + 画布图数据，再转 flovira 结构。
 */
function getFlowJson(): string {
  ...
  return logicFlowJsonToFlovira(logicJson.value);
}
...
/* ========== React 风格工作台 ========== */
.wf-designer-shell--workbench {
  box-sizing: border-box;
  height: 100%;
  min-height: 0 !important;
  padding: var(--wf-space, 12px);
  display: flex;
  flex-direction: column;
  overflow: hidden;
  background: var(--wf-bg-container, #f5f7fa) !important;
  color: var(--wf-text-primary, #303133);
  font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", "PingFang SC", "Helvetica Neue", "Microsoft YaHei", sans-serif;
  -webkit-font-smoothing: antialiased;
}

.wf-designer-shell--workbench .design-header {
  flex: 0 0 60px;
  min-height: 60px;
  ...
  border-radius: var(--wf-radius, 8px) var(--wf-radius, 8px) 0 0;
  box-shadow: var(--wf-shadow-sm);
  z-index: 4;
}

.wf-designer-shell--workbench .header-left {
  min-width: 0;
  max-width: 260px;
}

.wf-designer-shell--workbench .flow-name-wrapper {
  min-width: 0;
  gap: var(--wf-space-sm, 8px);
}

.wf-designer-shell--workbench .flow-brand-mark {
  width: 34px;
  height: 34px;
  display: inline-flex;
  align-items: center;
...
.wf-designer-shell--workbench .flow-brand-mark .svg-icon {
  width: 18px;
  height: 18px;
}

.wf-designer-shell--workbench .flow-name {
  max-width: 190px;
  padding: 0;
  border: 0;
  background: transparent;
...
.wf-designer-shell--workbench .flow-name:hover {
...
.flow-dirty-indicator {
  ...
  flex: 0 0 7px;
  border-radius: 50%;
  background: var(--wf-warning, #e6a23c);
}

.wf-designer-shell--workbench .steps-tabs {
  padding: 0;
  gap: var(--wf-space-lg, 20px);
  border-radius: 0;
  box-shadow: none;
  background: transparent;
}

.wf-designer-shell--workbench .step-tab {
  position: relative;
  min-height: 59px;
  padding: 0 var(--wf-space-xs, 4px);
  gap: 6px;
  border-radius: 0;
...
.wf-designer-shell--workbench .step-tab::after {
  content: '';
  position: absolute;
  left: 0;
  right: 0;
...
.wf-designer-shell--workbench .step-tab:hover,
.wf-designer-shell--workbench .step-tab.active,
.wf-designer-shell--workbench .step-tab.active:hover {
  color: var(--wf-primary, #409eff);
  background: transparent;
  box-shadow: none;
}

.wf-designer-shell--workbench .step-tab.active::after {
  background: var(--wf-primary, #409eff);
}

.wf-designer-shell--workbench .header-right {
  min-width: 0;
  align-items: center;
  gap: var(--wf-space-sm, 8px);
}

.wf-designer-shell--workbench .header-right > span,
.wf-designer-shell--workbench .workbench-header-group {
  display: inline-flex;
  align-items: center;
  gap: var(--wf-space-xs, 4px);
}

.wf-designer-shell--workbench .workbench-header-group--export {
  padding-left: var(--wf-space-sm, 8px);
  margin-left: var(--wf-space-xs, 4px);
  border-left: 1px solid var(--wf-border-lighter, #ebeef5);
}

.wf-designer-shell--workbench .workbench-header-group .el-button,
.wf-designer-shell--workbench .workbench-header-group button {
  width: 32px;
  height: 32px;
  padding: 0;
}
...
.wf-designer-shell--workbench .wf-design-workspace {
  flex: 1;
  min-height: 0;
  width: 100%;
  display: flex;
...
.wf-designer-shell--workbench .diagram-sidebar {
  position: relative;
  inset: auto;
  flex: 0 0 88px;
  width: 88px;
...
.wf-canvas-stage {
  ...
  min-height: 0;
  overflow: hidden;
  background: var(--wf-bg-container, #f5f7fa);
}

.wf-designer-shell--workbench .container {
  width: 100%;
  height: 100%;
  min-height: 0;
  border: 0;
...
.design-toolbar--canvas {
  position: absolute;
  left: var(--wf-space, 12px);
  bottom: var(--wf-space, 12px);
  z-index: 8;
  padding: var(--wf-space-xs, 4px);
  text-align: left;
  background: var(--wf-bg-white, #fff);
  border: 1px solid var(--wf-border-light, #e4e7ed);
  border-radius: var(--wf-radius, 8px);
...
.design-toolbar--canvas .toolbar-group {
  gap: 2px;
  padding: 0;
  margin: 0;
  border: 0;
...
.design-toolbar--canvas .el-button,
.design-toolbar--canvas button {
  width: 32px;
  height: 32px;
  padding: 0 !important;
}

.wf-designer-shell--workbench .logo-text {
  right: var(--wf-space-md, 16px);
  bottom: var(--wf-space-md, 16px);
  letter-spacing: 0;
}

html.dark .wf-designer-shell--workbench .wf-designer-body,
html.dark .wf-designer-shell--workbench .design-header,
html.dark .wf-designer-shell--workbench .diagram-sidebar {
  border-color: var(--wf-border-color, #333);
}

html.dark .wf-designer-shell--workbench .wf-design-workspace,
html.dark .design-toolbar--canvas {
