...
/** Import/export validation never translates legacy lifecycle meanings implicitly. */
export function validateLifecycleDefinition(definition: { ext?: unknown; listenerType?: string; listenerPath?: string; nodeList?: any[] }) {
  const validate = (item: { ext?: unknown; listenerType?: string; listenerPath?: string }, nodeType?: string) => {
    if ((!item.listenerType && item.listenerPath)
        || (item.listenerType || '').split(',').some(type => type && type.trim() !== 'formLoad')) {
      throw new Error('旧监听配置需迁移到 lifecycle；formLoad 单独保留')
    }
    validateLifecycle(parseLifecycle(lifecycleValue(item.ext)), nodeType)
  }
  validate(definition)
  for (const node of definition.nodeList || []) validate(node, String(node.nodeType))
}
