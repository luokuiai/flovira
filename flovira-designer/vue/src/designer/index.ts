...
import SubprocessProgress from '@/components/runtime/SubprocessProgress.vue'
import {
  setupDataProvider,
  setDataProvider,
  getDataProvider,
...
/** FloviraDesigner 插件安装选项。 */
export interface FloviraDesignerOptions {
  /** 全局组件尺寸（small / default / large）；等价于安装前调用 setComponentSize(size)。 */
  size?: ComponentSize
}

/**
 * Flovira 设计器「可复用层」统一出口（组件库 / npm 包入口）。
 *
 * 主入口 UI 库无关：不静态引入任何 UI 组件库（element-plus / ant-design-vue），
 * 消费方须显式注册一个 UI 适配器后再渲染设计器：
 *   import { FloviraDesigner, FlowDesigner, setUiAdapter, setDataProvider, createMockProvider } from '@luokuiai/flovira-vue-designer'
 *   import { elementPlusAdapter } from '@luokuiai/flovira-vue-designer/element-plus'  // 或 import { antdvAdapter } from '@luokuiai/flovira-vue-designer/antdv'
 *   import '@luokuiai/flovira-vue-designer/style'
 *   setUiAdapter(elementPlusAdapter)  // 选择 UI 库适配器，须在渲染 FlowDesigner 前调用
 *   app.use(FloviraDesigner)         // 注册 svg-icon + 中性组件 wf-*，图标零配置可用
 ...
 * 选择 element-plus（@luokuiai/flovira-vue-designer/element-plus）或 antdv（/antdv）适配器。
 * 图标走离线 iconify 集（ep + wf，见 src/icons），已在本模块加载时注册，零配置渲染，不依赖具体 UI 库图标。
 */
const install = (app: App, options: FloviraDesignerOptions = {}): void => {
...
const FloviraDesigner: Plugin = { install }

export {
  // 可复用画布组件（props 驱动，详见组件内 defineProps）
  FlowDesigner,
  // 全局图标组件（通常由 FloviraDesigner 插件自动注册，单独导出便于按需使用）
  SvgIcon,
  ...
  //   element-plus 适配器 import 自 @luokuiai/flovira-vue-designer/element-plus
  //   ant-design-vue 适配器 import 自 @luokuiai/flovira-vue-designer/antdv
  setUiAdapter,
  getUiAdapter,
  ...
  // 需先 app.use(FloviraDesigner) + setUiAdapter，依赖全局 wf-* 组件与 svg-icon
  BaseInfo,
  PropertySetting,
  DiagramSidebar,
  SubprocessProgress
