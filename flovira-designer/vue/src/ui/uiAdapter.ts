...
/**
 * 获取当前 UI 适配器。未注册时抛错，提示调用方先注册（app.use(FloviraDesigner) 会默认注册 Element Plus）。
 */
export function getUiAdapter(): UiAdapter {
  if (!currentAdapter) {
    throw new Error(
      '[flovira] UI 适配器未注册：请先 setUiAdapter(elementPlusAdapter) 或自定义适配器，' +
        '或 app.use(FloviraDesigner)（会默认注册 Element Plus 适配器）'
    )
  }
