...
/**
 * Flovira 设计器「组件库 / npm 包」构建配置。
 *
 * npm 组件库主入口构建，与各 UI 适配器子入口构建相互独立：
 ...
 *
 * 应用构建仍走 `bun run build:prod`；库构建走 `bun run build:lib`。
 *
 * @author warm
 */
  ...
  return {
      ...
      dts({
        ...
        afterDiagnostic: (diagnostics) => {
          if (diagnostics.length > 0) {
            throw new Error(`Declaration generation failed with ${diagnostics.length} type diagnostics`)
          }
        },
    ...
    build: {
      ...
      rollupOptions: {
        ...
        output: {
          assetFileNames: 'flovira-designer.[ext]'
        }
      }
    },
    css: {
