/**
 * Flovira 设计器本地 Mock 服务（开发联调用 · 零依赖）。
 *
 * 起一个真实 HTTP 服务（默认 :8080），对接 vite 的 /dev-api 代理：
 * 前端正常 `bun run dev` 访问 http://localhost:8083/（无需 ?mock=true），
 ...
 *
 * 启动：bun run mock            （= node mock/server.mjs，零依赖、无需安装任何包）
...
const routes = [
  // ===== 流程定义 =====
  ['POST', '/flovira/save-json', () => ok(true, '保存成功(mock)')],
  ['GET', '/flovira/query-def', () => ok({})],
  ['GET', '/flovira/query-def/:id', () => ok({})],
  ['GET', '/flovira/query-flow-chart/:id', () => ok({})],
  ['GET', '/flovira/handler-type', () => ok(['角色', '部门', '用户'])],
  ['GET', '/flovira/handler-result', (ctx) => {
  ...
  ['GET', '/flovira/handler-feedback', (ctx) => {
  ...
  // ===== 配置 =====
  ['GET', '/flovira-ui/config', () => ok({ tokenNameList: [], framework: 'springboot' })]
]
...
server.listen(PORT, () => {
  console.log('\n  Flovira Mock Server  (零依赖) 已启动')
  console.log(`  ->  http://localhost:${PORT}   （对接 vite /dev-api 代理 -> localhost:8080）`)
  console.log('  前端：bun run dev 后访问 http://localhost:8083/（无需 ?mock=true）即走真实 HTTP 链路')
