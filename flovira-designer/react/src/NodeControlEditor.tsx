  ...
  return <div className="frd-node-controls">
    <Field label="节点控制策略">
      <div className="frd-node-controls__permissions">
        {([
          ['allowRollback', '允许驳回'], ['allowTransfer', '允许转办'],
    ...
    {config.allowRollback && <>
      ...
      {config.rejectStrategy === 'TO_SPECIFIED_NODE' && <Field label="驳回目标节点">
        <Select ariaLabel="驳回目标节点" value={config.rejectTargetNodeCode} disabled={disabled}
      ...
      {['TO_DRAFT', 'TO_SPECIFIED_NODE', 'TO_REJECTOR_SPECIFIED_NODE'].includes(config.rejectStrategy) && <Field label="驳回后重新提交">
        <RadioGroup ariaLabel="驳回后重新提交" value={config.resubmitStrategy} disabled={disabled}
          options={[
            ...
            { value: 'CONTINUE_FROM_REJECTED_NODE', label: '从驳回节点继续' },
