...
import { createInitialDefinition, getSubmitterRule, setSubmitterRule, validateDefinition } from './model'
...
test('start and end have no tabs or form permissions, start defaults to all', () => {
  ...
  expect((view.getByLabelText('可提交人员') as HTMLSelectElement).value).toBe('ALL')
...
test.each(['USER', 'ROLE'])('submission %s uses host picker, preserves drafts and supports undo', async strategy => {
  const ref = createRef<ReactFlowDesignerRef>()
  const subject = { id: 'allowed', type: strategy, name: '允许提交' }
  const selector = vi.fn<ApproverSelector>().mockResolvedValue({ subjects: [subject] })
  const view = render(<ReactFlowDesigner ref={ref} capabilities={DEMO_CAPABILITIES} onSelectApprover={selector} />)
  const rule = () => getSubmitterRule(ref.current!.getDefinition().nodeList[0])
  fireEvent.click(view.getByRole('button', { name: '编辑节点：开始' }))
  fireEvent.change(view.getByLabelText('可提交人员'), { target: { value: strategy } })
  expect(view.queryByText('审批人为空时')).toBeNull()
  fireEvent.click(view.getByRole('button', { name: strategy === 'USER' ? '选择指定人员' : '选择指定角色' }))
  ...
  fireEvent.click(view.getByRole('button', { name: '编辑节点：开始' }))
  expect(view.getByText('允许提交')).toBeTruthy()
  fireEvent.change(view.getByLabelText('可提交人员'), { target: { value: 'ALL' } })
  fireEvent.click(view.getByRole('button', { name: '取消' }))
...
test('custom submission strategy uses the existing host picker and honors read-only state', async () => {
  const capabilities = { ...DEMO_CAPABILITIES, submitterStrategies: [{ code: 'PROJECT', version: 3,
    name: '项目成员', selectionType: 'RESOURCE' as const, resourceType: 'PROJECT', multiple: true, editorType: 'DIALOG' as const }] }
  const selector = vi.fn<ApproverSelector>().mockResolvedValue({ subjects: [{ id: 'p1', type: 'PROJECT' }] })
  const ref = createRef<ReactFlowDesignerRef>()
  const view = render(<ReactFlowDesigner ref={ref} capabilities={capabilities} onSelectApprover={selector} />)
  fireEvent.click(view.getByRole('button', { name: '编辑节点：开始' }))
  fireEvent.change(view.getByLabelText('可提交人员'), { target: { value: 'PROJECT' } })
  fireEvent.click(view.getByRole('button', { name: '选择项目成员' }))
  await waitFor(() => expect(view.getByText('p1')).toBeTruthy())
  fireEvent.click(view.getByRole('button', { name: '确定' }))
  expect(getSubmitterRule(ref.current!.getDefinition().nodeList[0]).strategyVersion).toBe(3)
  const saved = ref.current!.getDefinition()
  view.unmount()
  const readOnly = render(<ReactFlowDesigner defaultValue={saved} capabilities={capabilities} disabled />)
  fireEvent.click(readOnly.getByRole('button', { name: '编辑节点：开始' }))
  expect(readOnly.getByLabelText('可提交人员').hasAttribute('disabled')).toBe(true)
  expect(readOnly.getByRole('button', { name: '选择项目成员' }).hasAttribute('disabled')).toBe(true)
})
...
test('rejects malformed, unsupported and empty restricted rules instead of defaulting to all', () => {
  ...
  const validate = () => validateDefinition(definition, DEMO_CAPABILITIES).issues.filter(issue => issue.code === 'SUBMITTER_INVALID')
