...
本仓库（`flovira`，Dromara 开源轻量级工作流引擎 / SDK）的编码与协作规则，**以 [AGENTS.md](./AGENTS.md) 为唯一来源**。开始任何任务前先读根 `AGENTS.md`；若改动所在顶层模块下还有模块级 `AGENTS.md`，再读模块级。本文件只是核心红线摘要，**与 `AGENTS.md` 冲突时以 `AGENTS.md` 为准**。
...
- 这是**被他人依赖的开源类库 / SDK**（发布到 Maven 中央仓库，`groupId=com.luokuiai`），不是业务应用。
...
- `flovira-core`：框架 / ORM / JSON 无关的引擎核心（实体、服务、抽象 `FloviraDao`、`FlowEngine`、`FrameInvoker`、SPI、表达式 / 监听器策略）。
...
2. **解耦**：`flovira-core` 禁止 import `org.springframework.*` / `com.baomidou.*`；框架差异走 `FrameInvoker` + SPI + 各 starter，ORM 差异走 `FloviraDao` 实现。
...
10. **品牌与协议**：保留每个 Java 文件 Apache 2.0 license header、`@author warm`、`com.luokuiai` 包名、banner、README 链接与 `LICENSE`，不弱化「永久开源免费」声明。
...
- 本仓库无 `src/test`（测试在独立仓库 `flovira-test`），用 Gradle 分模块编译验证：`./gradlew :flovira-core:compileJava` 或 `./gradlew clean build`；core 改动至少编译一个下游 orm/plugin 模块。无法验证时如实报告命令、现象、风险与下一步。
- 未经用户明确批准：不 `git commit` / `git push`、不 `./gradlew publish` / 改版本推送、不改写历史、不执行 `DROP` / 批量 `DELETE`、不降 JDK 基线 / 删编译插件、不输出真实发布凭证。
