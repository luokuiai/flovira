...
`flovira` 是 [Dromara](https://dromara.org/) 社区的国产轻量级工作流引擎，`groupId=com.luokuiai`，发布到 Maven 中央仓库供他人依赖。它**是一个被集成的类库 / SDK，不是业务应用**：简洁轻量（核心仅 7 张表）、五脏俱全、扩展性强；后端通过 jar 集成引擎与设计器 API，前端通过独立 npm 包集成经典与仿钉钉双模式设计器。
