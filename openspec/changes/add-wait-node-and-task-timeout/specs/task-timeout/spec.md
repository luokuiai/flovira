...
The engine SHALL freeze timeout deadline, action, configuration, status and claim time on the current task and SHALL scan only active due tasks.

#### Scenario: Definition changes after task creation
- **WHEN** a definition's timeout configuration changes after a task was created
- **THEN** that task retains its original deadline and action
...
The timeout service SHALL atomically claim due tasks before execution, recover stale claims and reuse normal workflow transition services.

#### Scenario: Two scanners observe one due task
- **WHEN** two backend instances scan the same due task concurrently
- **THEN** at most one instance claims and executes its timeout action

#### Scenario: Claimed action fails
- **WHEN** the workflow transition throws after a task is claimed
- **THEN** the claim is released for retry and the failure is returned or logged explicitly

#### Scenario: Claim owner terminates
- **WHEN** a claim remains running past the configured claim timeout
