...
(
    `id`             bigint       NOT NULL COMMENT '主键id',
    `definition_id`  bigint          NOT NULL COMMENT '流程定义id',
    `source_node_code`  varchar(100)    NOT NULL COMMENT '跳转来源节点编码',
    `source_node_type`  tinyint(1)   DEFAULT NULL COMMENT '跳转来源节点类型（0开始节点 1中间节点 2结束节点 3互斥网关 4并行网关 5包容网关 6子流程 7等待）',
    `target_node_code` varchar(100)    NOT NULL COMMENT '跳转目标节点编码',
...
(
    ...
    `tenant_id`        varchar(40)                  DEFAULT NULL COMMENT '租户id',
    PRIMARY KEY (`id`) USING BTREE,
    KEY `idx_flow_his_task_instance_time` (`tenant_id`, `instance_id`, `created_at`)
...
(
    ...
    `associated_id`   bigint          NOT NULL COMMENT '任务表id',
    `created_at`  datetime    DEFAULT NULL COMMENT '创建时间',
    ...
    KEY `user_associated` (`associated_id`) USING BTREE
...
CREATE TABLE `flow_subprocess_run` (
    ...
    KEY `idx_subprocess_run_reconcile` (`run_status`, `updated_at`)
