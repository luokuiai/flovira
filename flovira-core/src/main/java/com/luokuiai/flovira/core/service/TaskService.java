 ...
 * @since 2023-03-29
 */
public interface TaskService extends IFloviraService<Task> {
    ...
    /**
     * 系统节点推进任务，复用普通任务的完整流转管线。
     *
     * @param flowParams 流程参数
     * @param task       当前任务
     * @return 流程实例
     */
    Instance skipSystemTask(FlowParams flowParams, Task task);
    ...
    List<Task> getByInsIdAndNodeType(Long instanceId, Integer nodeType);
     ...
     * @return 当前任务集合
     */
    List<Task> listByBusinessKey(String businessType, String businessId);

    List<Task> listDueTimeoutTasks(Date dueBefore, Date staleBefore, int limit);

    boolean claimTimeout(Long taskId, Date claimedAt, Date staleBefore);

    boolean claimWait(Long taskId, Date claimedAt);

    boolean releaseTimeout(Long taskId);
