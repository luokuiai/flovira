...
/**
 * 分管领导策略的业务实现基类。不提供人员来源，不自动注册。
 *
 * @author warm
 */
public abstract class AbstractSupervisingLeaderResolver implements ApproverResolver {
    @Override
    public final String getStrategy() {
        return ApproverStrategy.SUPERVISING_LEADER;
    }

    @Override
    public ApproverStrategyDefinition getDefinition() {
        return new ApproverStrategyDefinition().setCode(getStrategy()).setName("分管领导")
